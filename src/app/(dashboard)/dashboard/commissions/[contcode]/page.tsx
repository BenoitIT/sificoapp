...
import { commissionsContainHeaders } from "@/app/tableHeaders/commisions";
import { FaCcAmazonPay } from "react-icons/fa6";
import { useState } from "react";
import {
  commisionsEndpoint,
  getAllCommissionPerContainer,
  updateCommisionInfo,
...
const Page = () => {
  ...
  const container = params?.contcode;
  ...
  const { data, isLoading, error } = useSWR(
    [searchValues, currentPage, token,container],
    () => getAllCommissionPerContainer(container, searchValues, currentPage)
  );
  ...
  if (data?.data) {
    return (
      <div className="w-full">
        <CommissionSection
          headers={commissionsContainHeaders}
          data={data?.data}
          action={actions}
        />
